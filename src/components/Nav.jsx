...
const Nav = () => {
  ...
  const { ordersCount } = useSelector((state) => state.owner);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (cityRef.current && !cityRef.current.contains(event.target)) setIsCitySelectorOpen(false);
      if (cityRefMobile.current && !cityRefMobile.current.contains(event.target)) setIsCitySelectorOpen(false);
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between  items-center h-16">
          ...
          <div className="hidden lg:flex items-center justify-end space-x-6 flex-1">
            ...
            {/* Location Selector — visible only if user is NOT owner */}
            {userData?.role !== 'owner' && (
                ...
                {isCitySelectorOpen && (
                  <div className="absolute left-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-gray-200 z-50 p-2">
                        ...
                        .map((c) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 text-sm rounded-md hover:bg-gray-50 ${
                              location === c ? 'bg-orange-50 text-orange-600' : 'text-gray-700'
                            }`}
                          >
                            {c}
                          </button>
                    ...
                    <div className="flex items-center justify-between px-2 pt-2 border-t border-gray-100 mt-2">
                      <button type="button" onClick={requestCity} className="text-xs text-orange-600 hover:underline">
            ...
            {/* Search Bar — visible only if user is NOT owner */}
            {userData?.role !== 'owner' && (
            ...
            {/* Cart — visible only if user is NOT owner */}
            {userData?.role !== 'owner' && (
            ...
            {userData?.role === 'owner' && (
              <button
                ...
                className="flex items-center space-x-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg transition-colors font-medium text-sm"
              >
            ...
            <button className="relative text-gray-700 hover:text-orange-500 transition-colors font-medium text-sm">
              My Orders
              {userData?.role === 'owner' && ordersCount > 0 && (
                <span className="absolute -top-2 -right-3 bg-orange-500 text-white text-[10px] rounded-full h-5 min-w-5 px-1 flex items-center justify-center">
                  {ordersCount}
                </span>
              )}
            </button>
            ...
            <div className="relative" ref={profileRef} onClick={(e) => e.stopPropagation()}>
              <button onClick={toggleProfile} className="flex items-center space-x-2 text-gray-700 hover:text-orange-500 transition-colors">
                {userData?.profilePicture ? (
                  <img src={userData.profilePicture} alt="Profile" className="w-8 h-8 rounded-full object-cover" />
                ) : (
                  <div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center">
                    <FaUser className="text-gray-600" />
                  </div>
                )}
                <span className="hidden xl:block text-sm font-medium">{userData?.fullName || 'User'}</span>
              </button>
          ...
          <div className="lg:hidden">
            <button onClick={toggleMobileMenu} className="p-2 text-gray-700 hover:text-orange-500 transition-colors">
          ...
          <div className="lg:hidden border-t border-gray-200 py-4" ref={mobileMenuRef}>
            <div className="space-y-4">
              ...
              {userData?.role === 'owner' && (
                <button 
                  onClick={() => navigate('/add-food')}
                  className="w-full flex items-center space-x-3 p-3 bg-orange-500 hover:bg-orange-600 text-white rounded-lg transition-colors"
                >
              ...
              {/* My Orders with badge */}
              <button className="relative w-full flex items-center space-x-3 p-3 text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                <FaHistory className="text-xl" />
                <span className="font-medium">My Orders</span>
                {userData?.role === 'owner' && ordersCount > 0 && (
                  <span className="absolute right-3 bg-orange-500 text-white text-xs rounded-full h-5 min-w-5 px-1 flex items-center justify-center">
                    {ordersCount}
                  </span>
                )}
              </button>
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                <div className="space-y-2">
                  <button className="w-full flex items-center space-x-3 p-3 text-gray-700 hover:bg-gray-50 rounded-lg">
                    <FaUserCircle />
                    <span>Profile</span>
                  </button>
                  <button className="w-full flex items-center space-x-3 p-3 text-gray-700 hover:bg-gray-50 rounded-lg">
